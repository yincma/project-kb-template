...
Call `read_kb_source` only when the snippet is not enough to answer the question, and keep `max_chars` small. For visual search results, prefer `read_kb_result` with `indexed_source_path`, `attachment_path`, and `occurrence_id` so the Markdown summary is read instead of the original PDF/image bytes.
